...
res <- results(dds,name="condition_convex_vs_control")
...
res.05 <- results(dds, name="condition_convex_vs_control", alpha = 0.05)
...
resLFC1 <- results(dds, name="condition_convex_vs_control", lfcThreshold = 1)
summary(resLFC1)
...
head(resSig)
write.table(resSig, "cx.6h.sig.txt", sep = ';', col.names = NA)

resSig_up<-subset(resSig, resSig$log2FoldChange>0.5)
resSig_down<-subset(resSig, resSig$log2FoldChange<0.5)

write.table(resSig_up, "cx.6h.up.txt", sep = ';',col.names = NA)
write.table(resSig_down, "cx.6h.down.txt", sep = ';',col.names = NA)
